    ...
    for bomb in game_state["bombs"]:
        ...
        for number in range(1,BOMB_POWER):
            if bomb_y+number < ROWS:
                positions_danger[bomb_x][bomb_y+number] = _calculate_danger(time_passed, time_needed_to_explode, number)
            if bomb_y-number >= 0:
                positions_danger[bomb_x][bomb_y-number] = _calculate_danger(time_passed, time_needed_to_explode, number)
        # set vertically
        for number in range(1, BOMB_POWER):
            if bomb_x+number < COLS:
                positions_danger[bomb_x+number][bomb_y] = _calculate_danger(time_passed, time_needed_to_explode, number)
            if bomb_x-number >= 0:
                positions_danger[bomb_x-number][bomb_y] = _calculate_danger(time_passed, time_needed_to_explode, number)
        # set center
